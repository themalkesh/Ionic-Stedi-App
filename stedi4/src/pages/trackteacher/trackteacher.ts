...
/**
 * Generated class for the TrackteacherPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-trackteacher',
  templateUrl: 'trackteacher.html',
})
export class TrackteacherPage {
topics =  [
  {
...
;
  ...
  ionViewDidLoad() {
    ...
    //console.log(this.topics);
    //let localData = this.http.get('assets/parent.json');
    this.auth.getteachertracking();
    //.subscribe((data) => this.topics = data);
    // localData.subscribe((data: any) => {
    ...
    this.auth.teachertracking.valueChanges().subscribe((teachertracking) => {

      if (teachertracking) {
        this.topics = teachertracking.topics;
        //console.log(this.topics);
      }
    });
  ...
  updatetopic() {
    //console.log(this.topics);
    this.auth.updateteachertracking(this.topics).then((sucssess) => {
